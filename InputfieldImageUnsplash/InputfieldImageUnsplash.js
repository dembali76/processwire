$(document).ready(function () {
    ...
    let apiKey = "TXtTckxpjGAtu33DZ712disnUcTMpMqR-AdPepgS0pU",
        ...
        chosenColor,
        chosenOrder,
        moduleConfig = config.InputfieldImageUnsplash; // Config settings as defined by PageListPermissions.module

    $('.unsplashSearch').val($("#Inputfield_title2").val());
    $numGridImg = $(".gridImages .gridImage:not('.gridImage--delete')").length;

    $(document).on('click', '.unsplashButton', function(event) {
        let closestDataIdnrElement = $(event.target).closest('[data-idnr]');
        // Extract the value of the data-idnr attribute
        let dataIdnr = closestDataIdnrElement.data('idnr');
        // Reset pager to default values
        chosenOrientation = undefined;
        chosenColor = undefined;
        chosenOrder = undefined;
        event.preventDefault();
        $maxFiles = closestDataIdnrElement.find(".splashAndGrab").data('maxfiles');
        $uploadedFiles = closestDataIdnrElement.find(".splashAndGrab").data('uploadedfiles');
        sizesFound = 0;
        descsFound = 0;
        $results = $('.unsplashResults' + dataIdnr);
        $resultItems = $('.resultsItems' + dataIdnr);
        $chosen = closestDataIdnrElement.find('.unsplashChosen');
        $results.html('');
        field_name = $("#splashAndGrab" + dataIdnr).data('name');
        page_id = $("#splashAndGrab" + dataIdnr).data('id');
        $results.addClass('unsplashLoading');
    
        let query = $('#unsplashMagic' + dataIdnr).val();
        $button = $(event.target);
        getImages(query);
        return false;
    });
